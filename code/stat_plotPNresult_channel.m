function stat_plotPNresult_channel(Resfile,CMPfile,outputpath)
    try
        if contains(Resfile{1},'GLM.mat')
            if contains(Resfile{1},'*')
                Resdir = dir(Resfile{1});
            ...
            Resdir = dir(fullfile(Resfile{1},'GLM.mat'));
            if length(Resdir) > 1, error('L>1'); end
            Res = load(fullfile(Resdir.folder,Resdir.name));
        end
    catch ME
    ...
    for CHi = 1:length(CHANNELNAME)
        ...
        try
            if contains(CMPfile,'GLM.mat')
                if contains(CMPfile,'*')
                    Resdir = dir(CMPfile);
                    if length(Resdir) > 1, error('L>1'); end
                    cRes = load(fullfile(Resdir.folder,Resdir.name));
                else
                    cRes = load(CMPfile);
                end
            else
        ...
        % loop for get pvalue 
        if contains(outputpath,'perm')
            cRes = cRes.GLM.cmp;
        else
            cRes = cRes.GLM.sndLevel;
        end
        FBname = fieldnames(cRes.(CHANNELNAME{1}));
        FBname = FBname(contains(FBname,'contrast'));
        for FREQi = 1:length(FBname)
            figure(FREQi);
            cmpPriditor = cRes.(CHANNELNAME{CHi}).(FBname{FREQi}).LM.CoefficientNames;
            ...
            for CONDi = 1:length(CONDITION)
                ...
                % set(gcf, 'units','normalized','outerposition',[0 0 1 1]);  
                p = cRes.(CHANNELNAME{CHi}).(FBname{FREQi}).LM.Coefficients.pValue(string(cmpPriditor) == string(CONDITION{CONDi}));
                b = cRes.(CHANNELNAME{CHi}).(FBname{FREQi}).LM.Coefficients.Estimate(string(cmpPriditor) == string(CONDITION{CONDi}));
                if p < threshold
        ...
        for FREQi = 1:length(FrqName)
            ...
            saveas(gcf,fullfile(outputpath,[CHANNELNAME{CHi},'_',FrqName{FREQi},'.jpeg']))
        end
        close all;
    end
