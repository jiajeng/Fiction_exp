function stat_plotPNresult(Resfile,CMPfile,outputpath)
        ...
        if any(contains(channel_name,CHANNELNAME{CHi}))
            for FREQi = 1:length(FBname)
                ...
                for CONDi = 1:length(CONDITION)
                    ...
                    b = Res.(CHANNELNAME{CHi}).(FBname{FREQi}).LM.Coefficients.Estimate(string(cmpPriditor) == string(CONDITION{CONDi}));
                    if p < threshold
                        subplot(subplot_row,subplot_col,find(string(channel_name)==CHANNELNAME{CHi}))
                        ...
                        if b>0 % n>p
                            plot(sigX,sigY,'Color','b','LineWidth',1);
                        elseif b<0 % p>n
